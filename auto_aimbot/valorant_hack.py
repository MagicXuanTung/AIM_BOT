...
model = YOLO("../YOLO-Weights/best2024.pt").to(device)

# Use integer class indices in the classNames list
classNames = ["0"]  # Assuming 'enemyHead' is at index 2 in the YOLO model output

# Flags to control detection and mouse movement
detect_flag = True
mouse_move_flag = True
click_cooldown = 1.0  # Set the cooldown time in seconds
    ...
    if detect_flag:
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                cls = int(box.cls[0])
                print("Detected class index:", cls)  # Debugging statement

                if classNames[cls] == "0":  # Assuming 'enemyHead' is at index 2
                    ...
                    if mouse_move_flag:
                        print(f"Moving mouse to: {(x1 + x2) // 2}, {(y1 + y2) // 2}")  # Debugging statement
                        ...
                        if current_time - last_click_time >= click_cooldown:
                            print("Performing mouse click")  # Debugging statement
